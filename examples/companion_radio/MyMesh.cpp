...
MyMesh::MyMesh(mesh::Radio &radio, mesh::RNG &rng, mesh::RTCClock &rtc, SimpleMeshTables &tables, DataStore& store, AbstractUITask* ui)
  ...
  _prefs.utc_offset_hours = 1;  // default local correction (UTC+1)
...
// if name is provided as a build flag, use that as default node name instead
  ...
  _prefs.utc_offset_hours = static_cast<int8_t>(constrain(static_cast<int>(_prefs.utc_offset_hours), -12, 14));
...
bool MyMesh::syncRtcFromHeardAdverts(uint32_t& outEstimatedEpoch, uint8_t& outSamples) {
  ...
  estimates.reserve(total > 0 ? static_cast<size_t>(total) : 0);

  for (int i = 0; i < total; ++i) {
    ContactInfo c;
    if (!getContactByIdx(static_cast<uint32_t>(i), c)) {
      continue;
    }
    if (c.last_advert_timestamp < kMinReasonableEpoch) {
      continue;
    }
    if (now < c.lastmod) {
      continue;
    ...
    if (age > kMaxAdvertAge) {
      continue;
    }

    const uint64_t est64 = static_cast<uint64_t>(c.last_advert_timestamp) + static_cast<uint64_t>(age);
  ...
  outSamples = static_cast<uint8_t>(std::min<size_t>(estimates.size(), 255));
  outEstimatedEpoch = estimates[estimates.size() / 2];

  if (outEstimatedEpoch > now + kMinForwardApplyDelta) {
    rtc->setCurrentTime(outEstimatedEpoch);
    return true;
  }
  return false;
}
